...
| C3-5 | Macro | orphan detection is positionally undecidable; further heuristic patching is DECLINED (Two-Failure step 2) in favor of the documented Skeptic backstop | open level audit written before the next attempt | EVIDENCED | run 2026-07-11: EVO-004 failed as a false positive, EVO-005 as the mirror false negative — the foreign-vs-detached distinction cannot be decided from a single row. Decision logged in EVOLUTION_QUEUE.md "Différé" + spec documents the limit; the invariant "don't let a self-modifying system add fragility" holds. Re-verified 2026-08-02 under the stricter v3 evidence rule: `grep -n "Différé" EVOLUTION_QUEUE.md` -> line 132, `grep -c "Best-effort orphan detection" skill/wincreator/references/proof-ledger.md` -> 1, exit=0 |

## Cycle 4 — README "Bootstrap the evolution loop" section, gated

| ID | Level | Claim | Gate (what proves it) | Status | Evidence |
|----|-------|-------|------------------------|--------|----------|
| C4-1 | Meso | every bootstrap command in the README was executed in a virgin dir before publishing (no doc↔tool contradiction) | run the documented commands in a fresh project, cite output | EVIDENCED | run 2026-07-11: project A (no CLAUDE.md) -> 4 agents copied, CLAUDE.md created 38 lines, queue+catches created, `--self-test` -> `20/20 passed` exit 0 |
| C4-2 | Micro | the CLAUDE.md append is non-destructive (bimwin already has one) | run against a project with a pre-existing CLAUDE.md, check it survives | EVIDENCED | run 2026-07-11: project B pre-existing `# bimwin` first line preserved, "must survive" still present, lines 3 -> 41 (grew via `cat >>`, not replaced), WinCreator section appended once |
| C4-3 | Micro | the day-zero `--catches` STALE trap is demonstrated and documented, not left to surprise | run `--catches` on a fresh project log; state expectation in README + template | EVIDENCED | run 2026-07-11: `--catches ./SKEPTIC_CATCHES.md` on the fresh project -> `CATCHES STALE`, exit 1; README + templates/CLAUDE.evolution.md both state STALE-at-bootstrap is expected, `--self-test` is the install check |
| C4-4 | Meso | the two-SKEPTIC_CATCHES ambiguity is resolved in writing (skill = read-only heritage, project = live, opening reads both) | authority rule written into the shipped template | EVIDENCED | run 2026-07-11: `templates/CLAUDE.evolution.md` section "Which SKEPTIC_CATCHES.md is authoritative" states the rule; README points to it |
| C4-5 | Meso | the README's bootstrap is independently reproducible (a fresh Skeptic re-runs it) | Skeptic extracts the README commands and re-executes them from scratch | EVIDENCED | run 2026-07-11: independent Skeptic re-ran the README's 4 steps in virgin /tmp dirs -> 4 agents, CLAUDE.md 38 lines, `self-test: 20/20 passed` exit 0 (byte-for-byte), `--catches` STALE exit 1, append non-destructive incl. a no-trailing-newline host (template's leading blank line prevents gluing). Verdict EVIDENCED, no doc↔tool divergence |
